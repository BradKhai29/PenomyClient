...
async function updateArtworkDetail(artworkDetail, isCategoriesUpdated) {
    ...
    requestBody.append("animeId", artworkDetail.id);
    ...
    try {
        await axios({
            url: `${BaseWebApiUrl}/art17/anime/edit`,
...
async function getArtworkDetailById(artworkId) {
    try {
        const response = await axios({
            url: `${BaseWebApiUrl}/art17/anime/detail/${artworkId}`,
            method: HttpMethod.GET,
            headers: {
                Authorization: authStore.bearerAccessToken(),
            },
        ...
        artworkDetail.updatedAt = responseBody.updatedAt;
...
/**
 * Temporarily remove the artwork with specified input id.
 *
 ...
 */
async function temporarilyRemoveByIdAsync(artworkId) {
    const result = new TemporarilyRemoveArtworkResult(false, null);

    try {
        await axios({
            url: `${BaseWebApiUrl}/art8/temp-remove/${artworkId}`,
...
const EditAnimeDetailApiHandler = {
    ...
    temporarilyRemoveByIdAsync,
};

export { EditAnimeDetailApiHandler };
